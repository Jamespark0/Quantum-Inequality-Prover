...
def test_add_single_list_constraints_to_empty_constraint() -> None:
    ...
    assert (constraints_mach._constraints == np.array(constraint).reshape((-1, 2**n - 1))).all()
...
def test_add_multiple_constraints_in_list_to_empty_constraint() -> None:
    ...
    num_constraints = 5
    new_constraints = [[_ * (2**n - 1) + x for x in range(2**n - 1)] for _ in range(num_constraints)]
...
def test_add_multiple_constraints_array_to_empty_constraint() -> None:
    ...
    num_constraints = 5
    new_constraints = np.array([[_ * (2**n - 1) + x for x in range(2**n - 1)] for _ in range(num_constraints)])
    constraint_mach.add_constraints(new_constraints=new_constraints)
...
def test_single_constraint_to_non_empty_constraint() -> None:
    ...
    num_constraints = 5
    old_constraint = np.array([[_ * (2**n - 1) + x for x in range(2**n - 1)] for _ in range(num_constraints)])
    ...
    assert constraint_mach._constraints.shape == (og_shape[0] + 1, og_shape[1])
    assert (constraint_mach._constraints == np.vstack((old_constraint, new_constraint))).all()
...
def test_add_multi_constraints_list_to_non_empty_constraints() -> None:
    ...
    assert constraint_mach._constraints.shape == (og_shape[0] + num_constraints, og_shape[1])
    assert (constraint_mach._constraints == np.vstack((old_constraint, new_constraint))).all()
...
def test_add_multi_constraints_array_to_non_empty_constraints() -> None:
    ...
    num_constraints = 2
    new_constraint = np.array([[0 for _ in range(2**n - 1)] for x in range(num_constraints)])
    constraint_mach.add_constraints(new_constraints=new_constraint)
