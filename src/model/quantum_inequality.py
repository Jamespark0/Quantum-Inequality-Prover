...
class QuantumInequality:
    ...
    def __post_init__(self):
        ...
        self.index_to_pair = dict(
            (
                zip(
                    [x for x in range(1, len(self.entropic_space.all_pairs) + 1)],
    ...
    def _get_type_1_elemental_vector(self, i: int, j: int):
        remaining: frozenset[int] = self.entropic_space.universal - {i, j}

        elemental_i_j: NDArray = np.empty((0, len(self.entropic_space.all_pairs)))
        ...
        for r in range(len(remaining) + 1):
            for intersection in combinations(remaining, r):
                ...
                vector = np.zeros(len(self.entropic_space.all_pairs))
                vector[self.entropic_space.to_index[set_i]] = 1
                vector[self.entropic_space.to_index[set_j]] = 1
                vector[self.entropic_space.to_index[frozenset(set_i.union(set_j))]] = -1

                # Exclude intersections being empty
                if intersection != tuple():
                    vector[self.entropic_space.to_index[frozenset(intersection)]] = -1
    ...
    def _get_type_2_elemental_vector(self, k: int):
        ...
        i_diff_j = {
            (k + 1) % self.entropic_space.n if k + 1 > self.entropic_space.n else k + 1
        }

        remaining = self.entropic_space.universal - intersection - i_diff_j

        elemental_k = np.empty((0, len(self.entropic_space.all_pairs)))

        for r in range(0, len(remaining) + 1):
            for i_remaning in combinations(remaining, r):
                vec = np.zeros(len(self.entropic_space.all_pairs))
                ...
                vec[self.entropic_space.to_index[set_i]] = 1
                vec[self.entropic_space.to_index[set_j]] = 1
                vec[self.entropic_space.to_index[set_i.difference(set_j)]] = -1
                if j_remaining != frozenset():
                    vec[self.entropic_space.to_index[set_j.difference(set_i)]] = -1
    ...
    def _get_all_type_1(self):
        all_type_1: NDArray = np.empty((0, len(self.entropic_space.all_pairs)))
        for group in self.entropic_space.all_pairs:
            if len(group) > 2:
                break
            elif len(group) == 2:
                i, j = group
    ...
    def _get_all_type_2(self):
        all_type_2 = np.empty((0, len(self.entropic_space.all_pairs)))
        for k in range(1, self.entropic_space.n + 1):
