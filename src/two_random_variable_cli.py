def is_float(value: str) -> bool|float:
    try:
        num = float(value)
    except ValueError:
        return False
...
def is_valid_input(values):
    """
    Input a sequence and check if this is a valid input.

    A valid input consists of 3 and only 3 numerical values.

    This function aims to identify if a given string satisfy the condition
    ...
    Returns:
        ...
        If the sequence of values is a valid input, return the converted input. 
    ...
    raw_convert: list = [None, None, None]

    for i, element in enumerate(values):
        # if element cannot be converted to float
        decimal = is_float(element)

        if decimal is False:
            return False
        else:
            raw_convert[i] = decimal
